...
# Insert extra paths into sys.path
...
def attach(host=None):
    def auth_func():
        if host and host.startswith('localhost'):
            return ('foo', 'bar')  # Consider using environment variables for credentials
        else:
